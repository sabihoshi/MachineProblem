...
public:
	...
	VariadicTable(std::vector<std::string> headers,
		...
		: _headers(headers),
		  _num_columns(std::tuple_size_v<DataTuple>),
	...
	void print(StreamType& stream)
	{
		...
		for (unsigned int i = 0; i < _num_columns; i++)
		{
			...
			auto col_size = _column_sizes[i];
			stream << Repeat("═", col_size + _cell_padding * 2);
			if (i < _num_columns - 1)
				stream << "╦";
			else
				stream << "╗";
...
protected:
	...
	template <std::size_t I,
	          ...
	          typename StreamType,
	          typename = std::enable_if_t<
		          I != std::tuple_size_v<std::remove_reference_t<TupleType>>>>
	 ...
	 */
	template <class T>
	size_t sizeOfData(const T& data, decltype(static_cast<T*>(nullptr)->size())* /*dummy*/ = nullptr)
	{
		return data.size();
	}
	...
	size_t sizeOfData(const T& data,
		std::enable_if_t<std::is_integral_v<T>>* /*dummy*/ = nullptr)
	{
		if (data == 0)
			return 1;
